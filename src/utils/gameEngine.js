...
let isActiveNow = function (h, w, state, seen) {
  ...
  if (state.field[index] === state.toMove * 2) {
    return getAllNeibours(h, w, state.sizeH, state.sizeW)
      .map((neibIndex) =>
        isActiveNow(
          index2H(neibIndex, state.sizeW),
          index2W(neibIndex, state.sizeW),
          state,
          seen
        )
      )
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let k = 0; k < tmp_state.sizeW * tmp_state.sizeH; k++) {
      if (
        isStepValid(
          tmp_state,
          index2H(k, tmp_state.sizeW),
          index2W(k, tmp_state.sizeW)
        )
      ) {
        found = true;
